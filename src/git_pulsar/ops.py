...
def bootstrap_env() -> None:
    ...
    Optionally configures `direnv` for environment switching and generates
    VS Code settings based on the active configuration.
    ...
    if not shutil.which("uv"):
        missing.append("uv")
    if config.env.generate_direnv and not shutil.which("direnv"):
    ...
    if not (cwd / "pyproject.toml").exists():
        ...
        subprocess.run(
            ["uv", "init", "--no-workspace", "--python", config.env.python_version],
            check=True,
        )
    else:
    ...
    if config.env.generate_direnv:
        ...
        if not envrc_path.exists():
            ...
            envrc_content = textwrap.dedent(f"""\
                # Auto-generated by git-pulsar
                if [ ! -d "{config.env.venv_dir}" ]; then
                    echo "Creating virtual environment..."
                    uv sync
                fi
                source {config.env.venv_dir}/bin/activate
    ...
    # 4. VS Code Settings
    if config.env.generate_vscode_settings:
        ...
        if not settings_path.exists():
            ...
            settings_content = textwrap.dedent(f"""\
                {{
                    "python.defaultInterpreterPath": "{config.env.venv_dir}/bin/python",
                    "python.terminal.activateEnvironment": true,
                    "files.exclude": {{
                        "**/__pycache__": true,
                        "**/.ipynb_checkpoints": true,
                        "**/.DS_Store": true,
                        "**/{config.env.venv_dir}": true
                    }},
                    "search.exclude": {{
                        "**/{config.env.venv_dir}": true
                    }}
    ...
    if config.env.generate_direnv and "DIRENV_DIR" not in os.environ:
